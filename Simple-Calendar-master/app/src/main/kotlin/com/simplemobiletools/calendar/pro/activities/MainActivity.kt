...
import kotlinx.android.synthetic.main.activity_graph_btn.*
...
class MainActivity : SimpleActivity(), RefreshRecyclerViewListener {
    ...
    private var goTOGraphButton: MenuItem? = null
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        if(item.itemId==R.id.graph_view){
    ...
    @SuppressLint("NewApi")
    private fun getNewGraphShortcut(appIconColor: Int): ShortcutInfo {
        val newGraph = getString(R.string.new_task)

        val newTaskIntent = Intent(this, SplashActivity::class.java)
        newTaskIntent.action = SHORTCUT_NEW_TASK
        return ShortcutInfo.Builder(this, "newGraph")
            .setShortLabel(newGraph)
            .setLongLabel(newGraph)
    ...
    private fun showchart(){
        val items = arrayOf(getString(R.string.add_barchart),
             getString(R.string.add_piechart), )
        var selectedItemIndex : Int = 0

        val intentPie = Intent(this, ChartPieActivity::class.java)

        val builder = AlertDialog.Builder(this)
            .setSingleChoiceItems(items, 0){ _, which -> selectedItemIndex = which }
            .setPositiveButton("OK") { dialogInterface: DialogInterface, i: Int ->
                var selected: String
                if(selectedItemIndex == 0) updateView(WEEKLY_VIEW) else startActivity(intentPie)
